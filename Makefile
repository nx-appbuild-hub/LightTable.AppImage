...
all: clean
	...
	mkdir --parents $(PWD)/build/AppDir/lighttable

	wget --output-document=$(PWD)/build/build.tar.gz https://github.com/LightTable/LightTable/releases/download/0.8.1/lighttable-0.8.1-linux.tar.gz
	...
	cp --force --recursive $(PWD)/build/lighttable-*/* $(PWD)/build/AppDir/lighttable
	cp --force --recursive $(PWD)/AppDir/* $(PWD)/build/AppDir

	chmod +x $(PWD)/build/AppDir/AppRun
	chmod +x $(PWD)/build/AppDir/*.desktop

	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/AppDir $(PWD)/LightTable.AppImage
	chmod +x $(PWD)/LightTable.AppImage
